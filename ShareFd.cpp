...
static int FdRead(int chn,int* fd,void* readBuf,int bufLen,int timeOut)
{
	...
	*fd = -1;
	...
	{	
		...
		int sendBuffSize = 0;
		int ret = setsockopt(sockFds[chn], SOL_SOCKET, SO_RCVBUF, (void *) &sendBuffSize, sizeof(sendBuffSize));
		if(ret < 0)
		{
...
static int FdWrite(int chn,int fd, void* writeBuf, int writeLen)
{
	...
	{	
		...
		int sendBuffSize = 0;
		int ret = setsockopt(sockFds[chn], SOL_SOCKET, SO_SNDBUF, (void *) &sendBuffSize, sizeof(sendBuffSize));
		if(ret < 0)
		{
			LOG_ERROR("socket set sendbuf opt fail\n");
			return -1;
...
int main(int argc,char** argv)
{
	...
	else
	{
        ...
        {
            char buffer[1024] = {0};
            int len = FdRead(atoi(argv[1]),&fd, buffer, 1024, 2000);
            if(len > 0) 
			{
				LOG_ERROR("%s fd:%d\n",buffer,fd);
				close(fd);
